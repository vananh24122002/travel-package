<div class="table-main">
            ...
            <a href="index.php?action=addtourdm"><input type="button" value="Thêm mới" class="button-right"></div></a>
                <thead>
                  <tr>
                    <th scope="col"></th>
                    <th scope="col">Mã danh mục</th>
                    <th scope="col">Tên danh mục</th>
                    <th scope="col">Giới thiệu danh mục</th>
                    <th scope="col"></th>
                  </tr>
                </thead>
                <tbody id="load-dm">
            ...
            <input type="button" id="t1" value="Xóa tất cả"></div>
             ...
             <!-- phân trang -->
             <?php
                  $count = $count_tourdm['count_array'];
                  $pages = ceil($count/5);
            ...
            <nav aria-label="Page navigation example">
              <ul class="pagination1">
                <?php for($i = 1 ; $i<=$pages ; $i++){ ?>
                  <li class="page-item"><a class="page-link" href="index.php?action=listtourdm&pages=<?=$i?>"><?php echo $i ?></a></li>
                <?php } ?>
              </ul>
          </nav>
  ...
  //danh mục tin tức
  function load_dm_tour(){
          $.ajax({
            type: "POST",
            url: "Tour/DanhMuc/load_dm.php",
            cache: false,
